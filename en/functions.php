...
function lista_alimenti($TipoLista, $TP)
{
  ...
  //else {echo "<h4 class='bg-alert text-center text-light'>CODICE UTENTE MANCANTE. CREA UN NUOVO UTENTE</h4>" ;}
    ...
    echo "<h6 class='bg-alert text-center text-light'>MISSING USER CODE. REGISTER A NEW USER.</h6>";
  }
} // END FX
...
{ // $risultto sara cio che arriva dalla mysqli_query($connessioneDB, @sql);
    ...
    if ($username == ""  or $passwd == "") {
...

      {
        ...
        if ($rowcount == 0) {
          ...
          echo "<h4 class='bg-alert text-center text-light'>ERROR : USER NOT FOUND !</h4>" . mysqli_error($connessioneDB);
...

        {
          ...
          if ($id_user == 0) {
...

            {
              ...
              echo "<h4 class='bg-alert text-center text-light'>ERROR : ADMIN LEVEL NOT ENABLED !</h4>" . mysqli_error($connessioneDB);
            }
          } else    // id_user != 0 //
